#!/usr/bin/env python3
"""
生成今日股票推荐列表 - v3.0 纯技术指标版
基于五大策略核心优化 v2.0 的纯技术指标实现
"""
...
from strategy_v3_tech_only import TechOnlyStrategies
...
def generate_test_recommendations():
    ...
    output_path = '../test_output/今日推荐_v3.0_纯技术版_2026-02-24.csv'
    ...
    report_path = '../test_output/今日推荐_v3.0_纯技术版_2026-02-24.md'
...
def generate_markdown_report(recommendations, date):
    """生成 Markdown 格式的推荐报告"""
    report = f"""# A股 v3.0 策略今日推荐（纯技术指标版）
...
"""
    ...
    report += "\n## 🔍 策略说明（纯技术指标版）\n\n"
    report += "**价值策略**: 价格 vs 均线预期差 - 识别价格偏离均线的错误定价\n"
    report += "**产业策略**: 量价关系拐点 - 通过量价配合识别行业轮动信号\n"
    report += "**情绪策略**: RSI极端值反转 - 利用超买超卖指标进行逆向投资\n"
    report += "**趋势策略**: 均线多头排列加速 - 识别趋势强化的技术信号\n"
    report += "**量化策略**: 波动率收缩突破 - 捕捉变盘前兆的技术模式\n\n"
    
    report += "> **注意**: 此为纯技术指标版推荐，仅基于价格和成交量数据，适用于数据有限的场景。\n"
