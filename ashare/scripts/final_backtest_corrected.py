...
class CorrectedFinalBacktest:
    ...
    def load_and_align_data(self):
        """加载数据并对齐共同交易日"""
        print(f"Loading and aligning data from {self.data_path}...")
        ...
        for i, csv_file in enumerate(selected_files):
            ...
            try:
                ...
                # 确保有足够数据（至少120天，覆盖3个月+T+5）
                if len(df) >= 120:
                    self.stock_data[stock_code] = df
                    all_dates.update(df['日期'].dt.date)
                    valid_count += 1
                
                if (i + 1) % 20 == 0:
                    print(f"Loaded {valid_count} valid stocks...")
        ...
        self.common_dates = [d for d in all_dates if start_date <= d <= end_date]
        self.common_dates = [d for d in self.common_dates if d.weekday() < 5]  # 只保留工作日
        
        print(f"共同交易日范围: {min(self.common_dates)} 到 {max(self.common_dates)}")
        print(f"共同交易日数量: {len(self.common_dates)}")
    ...
    def execute_corrected_backtest(self) -> Dict:
        """执行修正版回测"""
        print(f"执行修正版回测: {min(self.common_dates)} 到 {max(self.common_dates)}")
        print(f"股票数量: {len(self.stock_data)}")
        print(f"交易日数量: {len(self.common_dates)}")
        
        backtest_results = {strategy: {'trades': [], 'win_rate': 0, 'avg_return': 0} 
                           for strategy in self.strategies}
        
        valid_trade_days = 0
        for i, trade_date in enumerate(self.common_dates):
            trade_datetime = datetime.combine(trade_date, datetime.min.time())
            
            try:
                top3_by_strategy = self.select_top3_by_strategy(trade_datetime)
                
                has_valid_trades = False
                for strategy, stocks in top3_by_strategy.items():
                    for stock in stocks:
                        return_rate, is_profit = self.backtest_t_plus_5(stock, trade_datetime)
                        if return_rate != 0.0:
                ...
                if (i + 1) % 10 == 0:
                    print(f"已处理 {i + 1}/{len(self.common_dates)} 个交易日 ({valid_trade_days} 天有有效交易)...")
        ...
        print(f"\n回测完成! 有效交易天数: {valid_trade_days}/{len(self.common_dates)}")
...
def main():
    ...
    backtester = CorrectedFinalBacktest()
    results = backtester.execute_corrected_backtest()
    ...
    # 保存结果
    output_path = "/Users/blastai/.openclaw/workspace-0011ai/ashare/corrected_recent_backtest_results.json"
    with open(output_path, 'w', encoding='utf-8') as f:
        json.dump(results, f, ensure_ascii=False, indent=2)
    
    # 备份到桌面
    desktop_path = "~/Desktop/asharereport/corrected_recent_backtest_results.json"
    os.system(f"cp {output_path} {desktop_path}")
    
    print(f"\n修正版回测结果已保存到: {output_path}")
