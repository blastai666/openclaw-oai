...
class FixedSimpleBacktest:
    ...
    def calculate_simple_scores(self, stock_df: pd.DataFrame, date: datetime) -> Dict[str, float]:
        ...
        try:
            ...
            value_score = current_price / ma20
            industry_score = current_price / ma10
    ...
    def execute_fixed_backtest(self) -> Dict:
        """执行修复版回测"""
        print(f"执行修复版回测...")
...
def main():
    ...
    backtester = FixedSimpleBacktest()
    if not backtester.stock_data or not backtester.trade_dates:
        print("❌ 数据加载失败，无法执行回测")
        return
    
    results = backtester.execute_fixed_backtest()
    ...
    # 保存结果
    output_path = "/Users/blastai/.openclaw/workspace-0011ai/ashare/fixed_recent_backtest_results.json"
    with open(output_path, 'w', encoding='utf-8') as f:
        json.dump(results, f, ensure_ascii=False, indent=2)
    
    # 备份到桌面
    desktop_path = "~/Desktop/asharereport/fixed_recent_backtest_results.json"
    ...
    summary_path = "~/Desktop/asharereport/fixed_recent_backtest_summary.json"
