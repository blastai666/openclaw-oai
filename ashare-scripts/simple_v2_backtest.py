...
class SimpleV2Backtest:
    ...
    def calculate_v2_scores(self, stock_df: pd.DataFrame, date) -> Dict[str, float]:
        """
        v2 核心优化评分 - 简化实现
    ...
    def backtest(self) -> Dict:
        """执行 T+5 回测"""
        print("\nStarting v2 backtest...")
        results = {s: {'returns': [], 'wins': 0, 'total': 0} for s in self.strategies}
        
        for i, date in enumerate(self.trade_dates[:-5]):
            # 为每个策略选 top3 股票
            stock_scores = {stock: self.calculate_v2_scores(df, date) for stock, df in self.stock_data.items()}
    ...
    def compare_v1_v2(self, v2_stats: Dict):
        """与 v1 对比"""
        print("\n" + "="*60)
        print("v1 vs v2 对比")
        print("="*60)
        ...
        print(f"\n{'策略':<10} {'v1 胜率':<10} {'v2 胜率':<10} {'改进':<10} {'v1 收益':<10} {'v2 收益':<10} {'改进':<10}")
        print("-" * 70)
        
        for strategy in self.strategies:
            if strategy in v2_stats and strategy in v1_stats:
                v1_wr = v1_stats[strategy]['win_rate']
                v2_wr = v2_stats[strategy]['win_rate']
                wr_imp = v2_wr - v1_wr
                
                v1_ar = v1_stats[strategy]['avg_return']
                v2_ar = v2_stats[strategy]['avg_return']
                ar_imp = v2_ar - v1_ar
                
                print(f"{strategy:<10} {v1_wr:>8.2%}   {v2_wr:>8.2%}   {wr_imp:>+8.2%}   {v1_ar:>8.4f}   {v2_ar:>8.4f}   {ar_imp:>+8.4f}")
...
if __name__ == "__main__":
    ...
    backtester.compare_v1_v2(stats)
    backtester.save_results(stats, "/Users/blastai/.openclaw/workspace-0011ai/ashare/v2_simple_backtest_results.json")
