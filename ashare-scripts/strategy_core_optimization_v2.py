...
class CoreOptimizedStrategies:
    ...
    def _calculate_industry_momentum_acceleration(self, stock_df: pd.DataFrame, date: datetime) -> float:
        ...
        try:
            ...
            # 近期动量（20 日）
            momentum_20d = recent_data.tail(20)['收盘'].pct_change(20).iloc[-1]
            
            # 中期动量（60 日）
            momentum_60d = recent_data.tail(60)['收盘'].pct_change(60).iloc[-1]
    ...
    def _calculate_supply_demand_tension(self, stock_df: pd.DataFrame, date: datetime) -> float:
        ...
        try:
            ...
            # 价格趋势
            price_change = recent_data['收盘'].pct_change(10).iloc[-1]
            
            # 成交量趋势
            volume_change = recent_data['成交量'].pct_change(10).iloc[-1]
    ...
    def _detect_emotion_turning_point(self, stock_df: pd.DataFrame, date: datetime) -> float:
        ...
        try:
            ...
            # 价格反转（前跌后涨）
            if len(recent_data) >= 5:
                prev_trend = (recent_data.iloc[-3]['收盘'] - recent_data.iloc[-6]['收盘']) / recent_data.iloc[-6]['收盘']
    ...
    def _calculate_trend_acceleration(self, stock_df: pd.DataFrame, date: datetime) -> float:
        ...
        try:
            ...
            # 近期斜率（10 日）
            slope_10d = recent_data.tail(10)['收盘'].pct_change(10).iloc[-1]
            
            # 中期斜率（30 日）
            slope_30d = recent_data.tail(30)['收盘'].pct_change(30).iloc[-1]
    ...
    def _calculate_popularity_score(self, stock_df: pd.DataFrame, date: datetime) -> float:
        ...
        try:
            ...
            # 涨跌幅吸引力
            price_change = recent_data['收盘'].pct_change(5).iloc[-1]
